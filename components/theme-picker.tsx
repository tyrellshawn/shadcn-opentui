...
function ThemePickerPanel({
  ...
  return (
    <div className="bg-black/95 text-primary">
      <div className="flex items-center gap-2 border-b border-primary/20 px-3 py-2">
        ...
        <input
          ...
          className="h-8 w-full bg-transparent text-sm outline-none placeholder:text-primary/45"
        />
      </div>

      <div className={cn("overflow-y-auto p-1", compact ? "max-h-[200px]" : "max-h-[280px]") }>
        {filteredThemes.length === 0 ? (
          <div className="py-5 text-center text-sm text-primary/55">No theme found.</div>
        ) : (
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {!compact && <div className="truncate text-xs text-primary/55">{theme.description}</div>}
                </div>
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          aria-expanded={open}
          className={cn(
            "w-[280px] justify-between border-primary/30 bg-black/50 text-primary hover:bg-primary/10 hover:text-primary",
            className,
          )}
        >
      ...
      <PopoverContent className="w-[280px] border-primary/30 p-0 backdrop-blur-xl">
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="h-8 gap-2 px-2 text-primary hover:bg-primary/10"
        >
      ...
      <PopoverContent className="w-[240px] border-primary/30 p-0 backdrop-blur-xl">
        <ThemePickerPanel
          value={value}
